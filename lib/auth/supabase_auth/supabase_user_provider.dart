...
class LandGoTravelSupabaseUser extends BaseAuthUser {
  LandGoTravelSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> landGoTravelSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LandGoTravelSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
