...
serve(async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      // NO enviar email de bienvenida automáticamente
      // Se enviará después del primer login exitoso

      // Redirigir a la app con éxito
      return new Response(null, {
        status: 302,
        headers: {
          'Location': 'https://localhost:55165/mainPage?confirmed=true'
        }
      })
    } catch (error) {
      console.error('Error confirming account:', error)
      return new Response('Error confirming account', { status: 500 })
    }
  }
